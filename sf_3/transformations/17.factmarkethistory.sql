...
WITH market_dates_daily AS (
    ...
    WHERE dm.dm_date >= DATE_SUB(CURDATE(), INTERVAL 10 YEAR) -- Filter for the last 10 years
...
quarters AS (
    ...
    WHERE f.fi_qtr_start_date >= DATE_SUB(CURDATE(), INTERVAL 10 YEAR) -- Filter for the last 10 years
),
...
high_date AS (
    ...
    INNER JOIN market_dates_daily mdd
        ...
        AND mdd.dm_date >= DATE_SUB(hl.dm_date, INTERVAL 3 YEAR) -- Narrow to the last 3 year for high
...
low_date AS (
    ...
    INNER JOIN market_dates_daily mdd
        ...
        AND mdd.dm_date <= hl.dm_date
        AND mdd.dm_date >= DATE_SUB(hl.dm_date, INTERVAL 3 YEAR) -- Narrow to the last  3 year for low
